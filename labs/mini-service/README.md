...
This project's own design notes call for a second recurring example: "a
small request-serving application, such as an HTTP key-value service,
used only when queueing, tail latency, concurrency, or off-CPU behavior
needs a more realistic workload than `cyclelab` provides."

**Not implemented in this phase.** Chapter 3's guided lab, which is the
first place this kind of workload is called for, currently uses
