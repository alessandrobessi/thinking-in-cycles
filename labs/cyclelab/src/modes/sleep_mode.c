...
static void *sleep_worker(void *arg) {
    sleep_worker_ctx_t *ctx = (sleep_worker_ctx_t *)arg;
    ctx->affinity_result = affinity_apply(ctx->opts->affinity_spec, ctx->index,
                                           ctx->opts->threads, &ctx->affinity_reason);

    double sleep_us = ctx->opts->sleep_us;
    struct timespec req;
    req.tv_sec = (time_t)(sleep_us / 1e6);
    req.tv_nsec = (long)((sleep_us - (double)req.tv_sec * 1e6) * 1000.0);
    ...
    for (;;) {
        ...
        ctx->counter += 1; /* small on-CPU work between sleeps */
        done += 1;

        if (ctx->target_cycles > 0) {
            if (done >= ctx->target_cycles) break;
        } else {
    ...
    ctx->cycles_done = done;
    return NULL;
}

static void print_json(FILE *out, const cyclelab_options_t *opts,
                        const cyclelab_hostinfo_t *host,
                        sleep_worker_ctx_t *ctxs, int nthreads,
                        double duration_actual_s, long long total_cycles,
    ...
    double throughput = (duration_actual_s > 0) ? ((double)total_cycles / duration_actual_s) : 0.0;

    fprintf(out, "{\n");
    fprintf(out, "  \"tool\": \"cyclelab\",\n");
    fprintf(out, "  \"version\": \"%s\",\n", CYCLELAB_VERSION);
    fprintf(out, "  \"mode\": \"sleep\",\n");
    ...
    fprintf(out, "    \"total_cycles\": %lld,\n", total_cycles);
    fprintf(out, "    \"throughput_cycles_per_s\": %.2f,\n", throughput);
    fprintf(out, "    \"threads\": [\n");
...
static void print_text(FILE *out, const cyclelab_options_t *opts,
    ...
    fprintf(out, "cyclelab sleep -- %s/%s, %d logical CPUs, build=%s\n",
            host->os, host->arch, host->logical_cpus, CYCLELAB_BUILD_TYPE);
    fprintf(out, "threads=%d sleep_us=%.3f\n", opts->threads, opts->sleep_us);
    for (int i = 0; i < nwarnings; i++) {
        fprintf(out, "warning: %s\n", warnings[i]);
    }
    fprintf(out, "duration_actual_s=%.6f total_cycles=%lld throughput_cycles_per_s=%.2f\n",
            duration_actual_s, total_cycles, throughput);
    for (int i = 0; i < nthreads; i++) {
        fprintf(out, "  thread[%d] cycles=%ld elapsed_s=%.6f affinity_applied=%s\n",
                ctxs[i].index, ctxs[i].cycles_done, ctxs[i].elapsed_s,
                (ctxs[i].affinity_result == CYCLELAB_AFFINITY_APPLIED) ? "true" : "false");
    }
    fprintf(out, "context_switches voluntary=%ld involuntary=%ld\n", ctx_nvcsw, ctx_nivcsw);
}
...
int sleep_run(const cyclelab_options_t *opts, const cyclelab_hostinfo_t *host) {
    ...
    sleep_worker_ctx_t *ctxs = calloc((size_t)nthreads, sizeof(sleep_worker_ctx_t));
    ...
    for (int i = 0; i < nthreads; i++) {
        ...
        ctxs[i].target_cycles = opts->iterations;
        ctxs[i].counter = 0;
    }

    for (int i = 0; i < nthreads; i++) {
        if (pthread_create(&tids[i], NULL, sleep_worker, &ctxs[i]) != 0) {
    ...
    for (int i = 0; i < nthreads; i++) {
        total_cycles += ctxs[i].cycles_done;
    }
    ...
    if (opts->format == CYCLELAB_FMT_JSON) {
        print_json(out, opts, host, ctxs, nthreads, duration_actual_s, total_cycles,
                   ctx_nvcsw, ctx_nivcsw, warnings, nwarnings);
    } else {
        print_text(out, opts, host, ctxs, nthreads, duration_actual_s, total_cycles,
                   ctx_nvcsw, ctx_nivcsw, warnings, nwarnings);
    }
