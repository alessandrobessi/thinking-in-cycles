...
int false_sharing_run(const cyclelab_options_t *opts, const cyclelab_hostinfo_t *host) {
    ...
    if (opts->format == CYCLELAB_FMT_JSON) {
        print_json(out, opts, host, ctxs, nthreads, duration_actual_s, total_increments, warnings, nwarnings);
    } else {
        print_text(out, opts, host, ctxs, nthreads, duration_actual_s, total_increments, warnings, nwarnings);
    }
